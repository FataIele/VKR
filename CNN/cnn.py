...
class CNN():
    ...
    def emnist_model(self):
        ...
        model.add(Dense(len(self.emnist_labels), activation='softmax'))
        model.compile(loss='categorical_crossentropy', optimizer='adadelta', metrics=['accuracy'])
        return model

    def emnist_model2(self):
        model = Sequential()
    ...
    def emnist_train(self, model, path):
        emnist_path = path
        ...
        x_train_cat = keras.utils.to_categorical(y_train, len(self.emnist_labels))
        y_test_cat = keras.utils.to_categorical(y_test, len(self.emnist_labels))
        ...
        history = model.fit(X_train, x_train_cat, validation_data=(X_test, y_test_cat), callbacks=[learning_rate_reduction], batch_size=64, epochs=3)
    ...
    def emnist_predict_img(self, model, img):
        ...
        return chr(self.emnist_labels[result[0]])
    ...
    def img_to_str(self, model: Any, image_file: str):
        letters = self.letters_extract(image_file)
        s_out = ""
        for i in range(len(letters)):
            dn = letters[i+1][0] - letters[i][0] - letters[i][1] if i < len(letters) - 1 else 0
            s_out += self.emnist_predict_img(model, letters[i][2])
            if (dn > letters[i][1]/4):
                s_out += ' '
        return s_out
    ...
    def main(self, path1="", path2=""):
        if path1!="":
            ...
            self.emnist_train(model, path1)
            model.save('emnist_letters.h5')
        if path2!="":
            model = keras.models.load_model('/kobzev/CNN/emnist_letters.h5')
            s_out = self.img_to_str(model, path2)
            self.results = s_out
